...
CATEGORIA_DE_PLANO = "Categoria de plano"
TIPO_DE_ACOMODACAO = "Tipo de acomodação"
...
TABLE_CONFIGS: Dict[str, TableConfig] = {
    "report.vw_regras_diarias_taxas_historico": TableConfig(
        ...
        business_key=["contrato_id", "contrato_nome_prestador", "contrato_nome_operadora", "Escopo", "Hierarquia", "Subclassificação Diárias / Taxas", TIPO_DE_PLANO, CATEGORIA_DE_PLANO, TIPO_DE_ACOMODACAO, TIPO_DE_ATENDIMENTO, "Principais itens de transposição", "Outros itens de transposição"],
        columns="contrato_id, contrato_regional, contrato_nome_prestador, contrato_nome_operadora, TO_CHAR(termo_data_inicio_vigencia, 'DD/MM/YYYY') AS \"Detalhes da Vigência\", conjunto_regra_indice + 1 AS \"Escopo\", regra_hierarquia AS \"Hierarquia\", regra_escopo_regra_de_transposicao AS \"Regra de Transposição\", regra_escopo_subclassificacao_diarias_taxas AS \"Subclassificação Diárias / Taxas\", regra_escopo_tipo_plano AS \"Tipo de plano\", regra_escopo_categoria_de_plano AS \"Categoria de plano\", regra_escopo_tipo_de_acomodacao AS \"Tipo de acomodação\", regra_escopo_tipo_atendimento AS \"Tipo de atendimento\", regra_escopo_principais_itens_de_transposicao AS \"Principais itens de transposição\", regra_escopo_outros_itens_de_transposicao AS \"Outros itens de transposição\", regra_tipo AS \"Tipo de Regra\", case when ((regra_tabela_tabela is not null) and (regra_tabela_versao is not null)) then concat(regra_tabela_tabela, ' - ', regra_tabela_versao) else '' end AS \"Tabela\", \
            case when ((regra_reajuste_tipo is null) or (case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end is null)) then '' else concat(regra_reajuste_tipo, ' de ', case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end, '%') end AS \"Reajuste %\", \
            case when ((regra_rsfd_taxa_comercializacao_tipo is null) or (case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end is null)) then '' else concat(regra_rsfd_taxa_comercializacao_tipo, ' de ', case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end, '%') end AS \"Taxa de Comercialização\", \
            regra_entrantes AS \"Entrantes\", regra_regra_de_entrantes AS \"Regra de Entrantes\", regra_periodicidade_entrantes AS \"Periodicidade entrantes\", regra_observacoes_entrantes AS \"Observações Entrantes\", regra_descontinuados AS \"Descontinuados\", regra_regra_de_descontinuados AS \"Regra de Descontinuados\", regra_periodicidade_descontinuados AS \"Periodicidade descontinuados\", regra_observacoes_descontinuados AS \"Observações Descontinuados\", regra_observacoes AS \"Observações\", regra_tipo_de_alteracao AS \"Tipo de Alteração\"",
    ...
    "report.vw_regras_hm_sadt_historico": TableConfig(
        ...
        business_key=["contrato_id", "contrato_nome_prestador", "contrato_nome_operadora", "Escopo", "Hierarquia", "Subclassificação HM/SADT", TIPO_DE_PLANO, CATEGORIA_DE_PLANO, TIPO_DE_ACOMODACAO, "Especialidades HM/SADT", TIPO_DE_ATENDIMENTO],
        columns="contrato_id, contrato_regional, contrato_nome_prestador, contrato_nome_operadora, TO_CHAR(termo_data_inicio_vigencia, 'DD/MM/YYYY') AS \"Detalhes da Vigência\", conjunto_regra_indice + 1 AS \"Escopo\", regra_hierarquia AS \"Hierarquia\", \
            regra_escopo_subclassificacao_hm_sadt AS \"Subclassificação HM/SADT\", regra_escopo_tipo_de_plano AS \"Tipo de plano\", regra_escopo_categoria_de_plano AS \"Categoria de plano\", regra_escopo_tipo_de_acomodacao AS \"Tipo de acomodação\", regra_escopo_especialidades_hm_sadt AS \"Especialidades HM/SADT\", \
            regra_escopo_tipo_de_atendimento AS \"Tipo de atendimento\", \
            regra_tipo AS \"Tipo de Regra\", \
            case when ((regra_tabela_tabela is not null) and (regra_tabela_versao is not null)) then concat(regra_tabela_tabela, ' - ', regra_tabela_versao) else '' end AS \"Tabela\", \
            case when ((regra_tabela_honorarios_tabela is null) or (regra_tabela_honorarios_versao is null)) then '' else concat(regra_tabela_honorarios_tabela, ' - ', regra_tabela_honorarios_versao) end AS \"Tabela (Honorários)\", \
            case when ((regra_catalogo_tabela is null) or (regra_catalogo_versao is null)) then '' else concat(regra_catalogo_tabela, ' - ', regra_catalogo_versao) end AS \"Catálogo\", \
            case when ((regra_tabela_de_portes_tabela is null) or (regra_tabela_de_portes_versao is null)) then '' else concat(regra_tabela_de_portes_tabela, ' - ', regra_tabela_de_portes_versao) end AS \"Tabela de Portes\", \
            case when ((regra_tabela_hm_serv3_tabela is null) or (regra_tabela_hm_serv3_versao is null)) then '' else concat(regra_tabela_hm_serv3_tabela, ' - ', regra_tabela_hm_serv3_versao) end AS \"Tabela HM SERV3\", \
            case when regra_tipo IN ('Tasa') then case when ((regra_reajuste_honorarios_tipo is null) or (case when regra_reajuste_honorarios_campo_protocolo = 'Ajuste' then regra_reajuste_honorarios_ajuste else regra_reajuste_honorarios_margem end is null)) then '' else concat(regra_reajuste_honorarios_tipo, ' de ', case when regra_reajuste_honorarios_campo_protocolo = 'Ajuste' then regra_reajuste_honorarios_ajuste else regra_reajuste_honorarios_margem end, '%') end else '' end AS \"Reajuste % (Honorários)\", \
            case when regra_tipo IN ('CBHPM') then case when ((regra_ajuste_porte_tipo is null) or (case when regra_ajuste_porte_campo_protocolo = 'Ajuste' then regra_ajuste_porte_ajuste else regra_ajuste_porte_margem end is null)) then '' else concat(regra_ajuste_porte_tipo, ' de ', case when regra_ajuste_porte_campo_protocolo = 'Ajuste' then regra_ajuste_porte_ajuste else regra_ajuste_porte_margem end, '%') end else '' end AS \"Ajuste Porte %\", \
            regra_especialidades_regras_de_multiplos AS \"Especialidades - Regras de Múltiplos\", \
            case when regra_tipo IN ('AMILPAR - REDE DOR', 'TASA - REDE DOR', 'TGA', 'THSM Bradesco', 'THSM Mediservice', 'Tabela Preço Fixo', 'Tabela Própria') then case when ((regra_reajuste_tipo is null) or (case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end is null)) then '' else concat(regra_reajuste_tipo, ' de ', case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end, '%') end else '' end AS \"Reajuste %\", \
            regra_fator_ch AS \"Fator CH\", \
            case when regra_tipo IN ('AMB', 'TASA - REDE DOR') then case when ((regra_ajuste_ch_tipo is null) or (case when regra_ajuste_ch_campo_protocolo = 'Ajuste' then regra_ajuste_ch_ajuste else regra_ajuste_ch_margem end is null)) then '' else concat(regra_ajuste_ch_tipo, ' de ', case when regra_ajuste_ch_campo_protocolo = 'Ajuste' then regra_ajuste_ch_ajuste else regra_ajuste_ch_margem end, '%') end else '' end AS \"Ajuste CH %\", \
            case when ((regra_tabela_sadt_tabela is null) or (regra_tabela_sadt_versao is null)) then '' else concat(regra_tabela_sadt_tabela, ' - ', regra_tabela_sadt_versao) end AS \"Tabela (SADT)\", \
            case when regra_tipo IN ('Tasa') then case when ((regra_reajuste_sadt_tipo is null) or (case when regra_reajuste_sadt_campo_protocolo = 'Ajuste' then regra_reajuste_sadt_ajuste else regra_reajuste_sadt_margem end is null)) then '' else concat(regra_reajuste_sadt_tipo, ' de ', case when regra_reajuste_sadt_campo_protocolo = 'Ajuste' then regra_reajuste_sadt_ajuste else regra_reajuste_sadt_margem end, '%') end else '' end AS \"Reajuste % (SADT)\", \
            regra_valor_uco AS \"Valor UCO\", \
            case when regra_tipo IN ('CBHPM') then case when ((regra_ajuste_uco_tipo is null) or (case when regra_ajuste_uco_campo_protocolo = 'Ajuste' then regra_ajuste_uco_ajuste else regra_ajuste_uco_margem end is null)) then '' else concat(regra_ajuste_uco_tipo, ' de ', case when regra_ajuste_uco_campo_protocolo = 'Ajuste' then regra_ajuste_uco_ajuste else regra_ajuste_uco_margem end, '%') end else '' end AS \"Ajuste % UCO\", \
            case when regra_tipo IN ('AMB', 'AMILPAR - REDE DOR', 'CBHPM', 'TASA - REDE DOR', 'TGA', 'THSM Bradesco', 'THSM Mediservice', 'Tasa') then case when ((regra_ajuste_porte_anestesico_tipo is null) or (case when regra_ajuste_porte_anestesico_campo_protocolo = 'Ajuste' then regra_ajuste_porte_anestesico_ajuste else regra_ajuste_porte_anestesico_margem end is null)) then '' else concat(regra_ajuste_porte_anestesico_tipo, ' de ', case when regra_ajuste_porte_anestesico_campo_protocolo = 'Ajuste' then regra_ajuste_porte_anestesico_ajuste else regra_ajuste_porte_anestesico_margem end, '%') end else '' end AS \"Ajuste % porte anestésico\", \
            regra_valor_filme AS \"Valor filme\", regra_observacoes AS \"Observações\", regra_tipo_de_alteracao AS \"Tipo de Alteração\"",
        ignore_columns=["termo_protocolo", "termo_data_criacao", "termo_protocolo_data_publicacao", "conjunto_regra_criado_por", "conjunto_regra_data_inclusao", "termo_ultima_versao"],
    ...
    "report.vw_regras_materiais_historico": TableConfig(
        ...
        business_key=["contrato_id", "contrato_nome_prestador", "contrato_nome_operadora", "Escopo", "Hierarquia", "Subclassificação Materiais", TIPO_DE_PLANO, TIPO_DE_ATENDIMENTO, "Especialidade"],
        ...
        regra_tipo AS \"Tipo de Regra\", case when ((regra_tabela_tabela is not null) and (regra_tabela_versao is not null)) then concat(regra_tabela_tabela, ' - ', regra_tabela_versao) else '' end AS \"Tabela\", \
        ...
        case when regra_tipo IN ('Brasíndice', 'Simpro', 'Tabela Preço Fixo', 'Tabela Própria') then case when ((regra_reajuste_tipo is null) or (case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end is null)) then '' else concat(regra_reajuste_tipo, ' de ', case when regra_reajuste_campo_protocolo = 'Ajuste' then regra_reajuste_ajuste else regra_reajuste_margem end, '%') end else '' end AS \"Reajuste %\", \
        case when regra_tipo IN ('Brasíndice', 'Valoração variavel') then case when ((regra_margem_tipo is null) or (case when regra_margem_campo_protocolo = 'Ajuste' then regra_margem_ajuste else regra_margem_margem end is null)) then '' else concat(regra_margem_tipo, ' de ', case when regra_margem_campo_protocolo = 'Ajuste' then regra_margem_ajuste else regra_margem_margem end, '%') end else '' end AS \"Margem %\", \
        case when regra_tipo IN ('Tabela Própria', 'Valoração variavel', 'Tabela Própria') then case when ((regra_rsfd_taxa_comercializacao_tipo is null) or (case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end is null)) then '' else concat(regra_rsfd_taxa_comercializacao_tipo, ' de ', case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end, '%') end else '' end AS \"Taxa de Comercialização\", \
    ...
    "report.vw_regras_pacotes_historico": TableConfig(
        ...
        business_key=["contrato_id", "contrato_nome_prestador", "contrato_nome_operadora", "Escopo", "Hierarquia", "Subclassificação Pacotes", TIPO_DE_PLANO, CATEGORIA_DE_PLANO, TIPO_DE_ACOMODACAO, TIPO_DE_ATENDIMENTO],
        columns="contrato_id, contrato_regional, contrato_nome_prestador, contrato_nome_operadora, TO_CHAR(termo_data_inicio_vigencia, 'DD/MM/YYYY') AS \"Detalhes da Vigência\", conjunto_regra_indice + 1 AS \"Escopo\", regra_hierarquia AS \"Hierarquia\", regra_escopo_subclassificacao_pacotes AS \"Subclassificação Pacotes\", regra_escopo_tipo_plano AS \"Tipo de plano\", regra_escopo_categoria_de_plano AS \"Categoria de plano\", regra_escopo_tipo_de_acomodacao AS \"Tipo de acomodação\", regra_escopo_tipo_atendimento AS \"Tipo de atendimento\", regra_tipo AS \"Tipo de Regra\", case when ((regra_tabela_tabela is not null) and (regra_tabela_versao is not null)) then concat(regra_tabela_tabela, ' - ', regra_tabela_versao) else '' end AS \"Tabela\", case when ((regra_reajuste_tipo is null) or (regra_reajuste_margem is null)) then '' else concat(regra_reajuste_tipo, ' de ', regra_reajuste_margem, '%') end AS \"Reajuste %\", case when ((regra_rsfd_taxa_comercializacao_tipo is null) or (regra_rsfd_taxa_comercializacao_margem is null)) then '' else concat(regra_rsfd_taxa_comercializacao_tipo, ' de ', regra_rsfd_taxa_comercializacao_margem, '%') end AS \"Taxa de Comercialização\", regra_entrantes AS \"Entrantes\", regra_regra_de_entrantes AS \"Regra de Entrantes\", regra_periodicidade_entrantes AS \"Periodicidade entrantes\", regra_observacoes_entrantes AS \"Observações Entrantes\", regra_descontinuados AS \"Descontinuados\", regra_regra_de_descontinuados AS \"Regra de Descontinuados\", regra_periodicidade_descontinuados AS \"Periodicidade descontinuados\", regra_observacoes_descontinuados AS \"Observações Descontinuados\", regra_observacoes AS \"Observações\", regra_tipo_de_alteracao AS \"Tipo de Alteração\"",
        ignore_columns=["termo_protocolo", "termo_data_criacao", "termo_protocolo_data_publicacao", "conjunto_regra_criado_por", "conjunto_regra_data_inclusao", "termo_ultima_versao"],
