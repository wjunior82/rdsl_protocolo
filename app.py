...
TABLE_CONFIGS: Dict[str, TableConfig] = {
    ...
    "report.vw_regras_hm_sadt_historico": TableConfig(
        ...
        case when regra_tipo IN ('Tasa') then case when ((regra_reajuste_honorarios_tipo is null) or (regra_reajuste_honorarios_margem is null)) then '' else concat(regra_reajuste_honorarios_tipo, ' de ', regra_reajuste_honorarios_margem, '%') end else '' end AS \"Reajuste % (Honorários)\", \
        case when regra_tipo IN ('CBHPM') then case when ((regra_ajuste_porte_tipo is null) or (regra_ajuste_porte_margem is null)) then '' else concat(regra_ajuste_porte_tipo, ' de ', regra_ajuste_porte_margem, '%') end else '' end AS \"Ajuste Porte %\", \
        regra_especialidades_regras_de_multiplos AS \"Especialidades - Regras de Múltiplos\", \
        case when regra_tipo IN ('AMILPAR - REDE DOR', 'TASA - REDE DOR', 'TGA', 'THSM Bradesco', 'THSM Mediservice', 'Tabela Preço Fixo', 'Tabela Própria') then case when ((regra_reajuste_tipo is null) or (regra_reajuste_margem is null)) then '' else concat(regra_reajuste_tipo, ' de ', regra_reajuste_margem, '%') end else '' end AS \"Reajuste %\", \
        regra_fator_ch AS \"Fator CH\", \
        case when regra_tipo IN ('AMB', 'TASA - REDE DOR') then case when ((regra_ajuste_ch_tipo is null) or (regra_ajuste_ch_margem is null)) then '' else concat(regra_ajuste_ch_tipo, ' de ', regra_ajuste_ch_margem, '%') end else '' end AS \"Ajuste CH %\", \
        ...
        case when regra_tipo IN ('Tasa') then case when ((regra_reajuste_sadt_tipo is null) or (regra_reajuste_sadt_margem is null)) then '' else concat(regra_reajuste_sadt_tipo, ' de ', regra_reajuste_sadt_margem, '%') end else '' end AS \"Reajuste % (SADT)\", \
        regra_valor_uco AS \"Valor UCO\", \
        case when regra_tipo IN ('CBHPM') then case when ((regra_ajuste_uco_tipo is null) or (regra_ajuste_uco_margem is null)) then '' else concat(regra_ajuste_uco_tipo, ' de ', regra_ajuste_uco_margem, '%') end else '' end AS \"Ajuste % UCO\", \
        case when regra_tipo IN ('AMB', 'AMILPAR - REDE DOR', 'CBHPM', 'TASA - REDE DOR', 'TGA', 'THSM Bradesco', 'THSM Mediservice', 'Tasa') then case when ((regra_ajuste_porte_anestesico_tipo is null) or (regra_ajuste_porte_anestesico_margem is null)) then '' else concat(regra_ajuste_porte_anestesico_tipo, ' de ', regra_ajuste_porte_anestesico_margem, '%') end else '' end AS \"Ajuste % porte anestésico\", \
