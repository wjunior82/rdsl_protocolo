...
TABLE_CONFIGS: Dict[str, TableConfig] = {
    ...
    "report.vw_regras_materiais_historico": TableConfig(
        ...
        columns="contrato_id, contrato_regional, contrato_nome_prestador, contrato_nome_operadora, TO_CHAR(termo_data_inicio_vigencia, 'DD/MM/YYYY') AS \"Detalhes da Vigência\", conjunto_regra_indice + 1 AS \"Escopo\", regra_hierarquia AS \"Hierarquia\", regra_escopo_subclassificacao_materiais AS \"Subclassificação Materiais\", regra_escopo_tipo_plano AS \"Tipo de plano\", regra_escopo_tipo_atendimento AS \"Tipo de atendimento\", regra_escopo_especialidade AS \"Especialidade\", \
            ...
            case when regra_tipo IN ('Tabela Própria', 'Valoração variavel') then case when ((regra_rsfd_taxa_comercializacao_tipo is null) or (case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end is null)) then '' else concat(regra_rsfd_taxa_comercializacao_tipo, ' de ', case when regra_rsfd_taxa_comercializacao_campo_protocolo = 'Ajuste' then regra_rsfd_taxa_comercializacao_ajuste else regra_rsfd_taxa_comercializacao_margem end, '%') end else '' end AS \"Taxa de Comercialização\", \
